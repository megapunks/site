...
export default function AdminDashboard() {
  ...
  const fetchTokenInfo = async () => {
    try {
      ...
      for (let id = 0; id < 50; id++) {
        try {
          const [uri, supply, minted, enabled, price] = await Promise.all([
            contract.uri(id),
            contract.maxSupply(id),
            contract.totalMinted(id),
            contract.mintEnabled(id),
  ...
  const exportPlayers = async () => {
    try {
      ...
      const rows = data.map(
        (p: any, i: number) =>
          `${i + 1},${p.address},${p.xp},${p.level},${p.feeds},${p.missed}`
      );
    ...
    <div className="p-6 max-w-7xl mx-auto text-yellow-200 font-pixel">
      ...
      <div className="bg-[#1e1b4b] p-6 rounded-xl border border-yellow-400 mb-10">
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <button disabled={loading} onClick={setTokenURI} className="button-pixel bg-blue-500 text-white px-4 py-2">📦 Update CID</button>
          <button disabled={loading} onClick={setMax} className="button-pixel bg-purple-500 text-white px-4 py-2">🔢 Update Supply</button>
          <button disabled={loading} onClick={setPriceETH} className="button-pixel bg-yellow-500 text-black px-4 py-2">💰 Update Price</button>
          <button disabled={loading} onClick={enableMint} className="button-pixel bg-green-600 text-white px-4 py-2">✅ Enable Mint</button>
          <button disabled={loading} onClick={disableMint} className="button-pixel bg-red-600 text-white px-4 py-2">🚫 Disable Mint</button>
          <button disabled={loading} onClick={exportHolders} className="button-pixel bg-indigo-600 text-white px-4 py-2">📤 Export Holders</button>
        </div>
      </div>
